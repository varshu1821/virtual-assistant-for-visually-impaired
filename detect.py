import io
import google.cloud.dialogflow_v2 as google_cloud_dialogflow
...
def detect_intent_texts(project_id, session_id, texts, language_code):
    ...
    session_client = google_cloud_dialogflow.SessionsClient()

    session = session_client.session_path(project_id, session_id)
    print('Session path: {}\n'.format(session))

    for text in texts:
        text_input = google_cloud_dialogflow.types.TextInput(
            text=text, language_code=language_code)

        query_input = google_cloud_dialogflow.types.QueryInput(text=text_input)
